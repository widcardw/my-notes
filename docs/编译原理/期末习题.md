...
终态 $Y \notin V_\mathrm{N}$，有限自动机 $M=\{ \{ Z, A, Y \}, \{ 0, 1 \}, \delta, \{Z\}, \{Y\} \}$，其中
...
给出项目集 $I_0$ 和 $I_0$ 接受 $L$ 推导出的 $I_1$

```ad-example
title: 解答

![[public/compile/compxthrwe.svg]]
...
A \to \; & (A_1) { \color{blue} \{ A \text{.s} = A_1 \text{.s} + 1 \} }  \\
...
$$
\begin{aligned}
S & \to \text{do } {\color{red}M}S_1 \text{ while}({\color{red}N}E) {\color{red}W}
\end{aligned}
$$
