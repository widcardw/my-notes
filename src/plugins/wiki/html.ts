import { SupportedTypes, detectSupportedFormat } from "./isSupportedFileFormat";
...
function html(opts: HtmlOptions = {}) {
  ...
  function exitWikiLink(token) {
    ...
    if (isEmbed) {
      const [cat, ext] = detectSupportedFormat(target);
      switch (cat) {
        ...
        case SupportedTypes.Img: {
          this.tag(
            `<img src="${hrefTemplate(link)}" alt="${displayName}" class="${classNames}" />`
          );
          break
        }
        ...
        case SupportedTypes.Audio: {
          this.tag(
            `<${cat} src="${hrefTemplate(link)}" alt="${displayName}" class="${classNames}" controls />`
          )
        }
